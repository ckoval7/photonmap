function KismetDataSource(name) {
...
Object.defineProperties(KismetDataSource.prototype, {
  //The below properties must be implemented by all DataSource instances

  /**
   * Gets a human-readable name for this instance.
   * @memberof KismetDataSource.prototype
   * @type {String}
   */
  name: {
    get: function () {
...
KismetDataSource.prototype.loadUrl = function (url, user, passwd) {
  let headers= {
 ...
 */
KismetDataSource.prototype.load = function (data) {
  ...
  for (let dev of data) {
    ...
    && dev['kismet.device.base.type'] !== "Airplane"
    && dev['kismet.device.base.type'] !== "Helicopter") {
      let longitude = dev['kismet.common.location.geopoint'][0].toFixed(5);
      let latitude = dev['kismet.common.location.geopoint'][1].toFixed(5);
      ...
      let devname = "";
      if(dev['kismet.device.base.name'] !== "") {
        devname = dev['kismet.device.base.name'];
      } else {
        devname = dev['kismet.device.base.macaddr'];
      }

      if (dev['kismet.device.base.type'].includes("BTLE")) {
        ...
        `;
      } else if(dev['kismet.device.base.type'].includes("BR/EDR")) {
        ...
        <tr>
          <td>${devname}</td>
          <td>${dev['kismet.device.base.macaddr']}</td>
          <td>${dev['kismet.device.base.type']}</td>
          <td>${dev['kismet.common.signal.max_signal']}</td>
        </tr>
        `;
      }
      ...
      //   Device Name: ${dev['kismet.device.base.name']}<br>
      //   Type: ${dev['kismet.device.base.type']}<br>
      //   Encryption: ${dev['kismet.device.base.crypt']}<br>
      //   MAC Address: ${dev['kismet.device.base.macaddr']}<br>
      //   Max RSSI: ${dev['kismet.common.signal.max_signal']}
      // </div>
      // `;
    }
  }
      ...
      <div class="collapsed">
        <table style="width:100%">
          <tr>
            ...
            <th onclick="sortTable(2)">Encryption</th>
...
KismetDataSource.prototype._setLoading = function (isLoading) {
